<template>
    <div ref="root" class="relative">
        ...
        <div
            v-if="open"
            class="absolute end-0 mt-1.5 w-36 bg-white rounded-lg shadow-lg border border-gray-100 z-50 overflow-hidden"
        >
            <button
                v-for="l in locales"
                :key="l.code"
                type="button"
                class="w-full text-start px-3 py-2 text-sm hover:bg-gray-50 transition-colors"
                :class="l.code === current?.code ? 'text-indigo-600 font-medium' : 'text-gray-600'"
                @click="choose(l.code)"
            >
                {{ l.label }}
            </button>
