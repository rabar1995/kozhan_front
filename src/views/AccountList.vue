    ...
    <div class="space-y-4">
            ...
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <h2 class="font-bold text-gray-900 text-sm sm:text-base">{{ $t('nav.wallets') }}</h2>
                        <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">
                            {{ sharedWallets.length }}
                        </span>
                    </div>
                    <p class="text-xs text-gray-400 mt-0.5 hidden sm:block">{{ $t('accounts.sharedIntro') }}</p>
                </div>
            </div>
        ...
        >
            <div
                ...
                :key="account.id"
                class="bg-white rounded-2xl border border-gray-200/90 shadow-xs hover:shadow-lg hover:border-indigo-200 transition-all duration-200 p-5 flex flex-col justify-between group relative overflow-hidden"
            >
                <div>
                    <!-- Card Top Header -->
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-center gap-3 min-w-0">
                            <div class="w-10 h-10 rounded-xl bg-indigo-50/80 text-indigo-600 flex items-center justify-center shrink-0 ring-1 ring-indigo-500/10">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 12a2.25 2.25 0 00-2.25-2.25H15a3 3 0 110-6h.75A2.25 2.25 0 0018 1.5H6A2.25 2.25 0 003.75 3.75v16.5A2.25 2.25 0 006 22.5h12a2.25 2.25 0 002.25-2.25V12zM15 12a1.5 1.5 0 100-3 1.5 1.5 0 000 3z" />
                                </svg>
                            </div>
                    ...
                    <div class="mt-4 p-3.5 rounded-xl bg-gray-50/80 border border-gray-100">
                        <div class="flex items-center justify-between text-[11px] font-semibold uppercase tracking-wider text-gray-400 mb-1">
                            ...
                            <span class="inline-flex items-center gap-1 text-[10px] text-indigo-600 bg-indigo-50 px-1.5 py-0.2 rounded font-medium border border-indigo-100">
                                {{ $t('accounts.officeShared') }}
                            </span>
                        </div>
                        <div
                ...
                <div class="mt-5 pt-3 border-t border-gray-100 flex items-center justify-between gap-2">
                    <button
                        class="w-full py-1.5 px-3 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors shadow-2xs"
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    <tr v-for="account in filteredWallets" :key="account.id" class="hover:bg-gray-50/80 transition-colors">
                        <td class="px-4 py-3 font-bold text-gray-900">{{ account.name }}</td>
        ...
        <Modal :open="showLedger" :title="`${$t('accounts.ledgerBtn')} — ${ledgerAccount?.name || ''}`" max-width="sm:max-w-3xl" @close="showLedger = false">
            <div class="flex gap-3 mb-4">
                ...
                <button class="px-4 py-2 text-sm rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 font-medium" @click="loadLedger(1)">
                    {{ $t('common.apply') }}
                </button>
            </div>
        ...
        <!-- ═══════ CREATE CASH SAFE MODAL ═══════ -->
        <Modal :open="showCreate" :title="$t('accounts.createSafeTitle')" @close="showCreate = false">
            <form class="space-y-4" @submit.prevent="create">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <select v-model="form.account_type_id" required class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500">
                            ...
                            <option v-for="t in cashAccountTypes" :key="t.id" :value="t.id">{{ t.name }} ({{ t.code }})</option>
                        </select>
                        <p v-if="errors.account_type_id" class="text-xs text-red-600 mt-1">{{ errors.account_type_id[0] }}</p>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <input v-model.number="form.opening_balance" type="number" step="0.0001" min="0" inputmode="decimal"
                        :placeholder="'0'" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" />
...
const CASH_TYPE_CODES = ['cash_safe', 'e_wallet', 'bank_account']
const cashAccountTypes = computed(() => accountTypes.value.filter((t) => CASH_TYPE_CODES.includes(t.code)))
...
const sharedWallets = computed(() => accountStore.accounts.filter((a) => a.visibility === 'office_shared'))

const filteredWallets = computed(() => {
    let list = sharedWallets.value
...
async function create() {
    ...
    error.value = ''
    try {
        const opening = Number(form.opening_balance)
